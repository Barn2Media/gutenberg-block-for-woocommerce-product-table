  ...
  wp.blocks.registerBlockType('barn2/wc-product-table', {
    ...
    edit: function edit(props) {
      ...
      if (typeof wcptbInvalid !== 'undefined') {
        var messageSplit = wcptbInvalid.message.split('%s'),
            message;

        if (messageSplit.length > 1) {
          message = [messageSplit[0], el('a', {
            href: wcptbInvalid.link,
            target: '_blank'
          }, wcptbInvalid.link_text), messageSplit[1]];
        } else {
          message = messageSplit[0];
        }

        blockStructure = el(Placeholder, {
          icon: tableIcon,
          label: 'Product Table',
          instructions: message
        });
      } else {
        blockStructure = el(Fragment, null, [el(InspectorControls, null, [el(SettingsPanel, {
          ...
          icon: tableIcon,
          alt: ''
        }), __('Product Table', 'wpt-block')]), el('div', {
          className: 'components-placeholder__fieldset'
        }, [el('span', {
          className: 'block-description'
        }, [__('Lists products in a table view using the WooCommerce Product Table plugin. ', 'wpt-block'), el('a', {
          href: 'https://barn2.co.uk/kb/woocommerce-product-table-gutenberg/',
          target: '_blank'
        }, __('Documentation', 'wpt-block'))]), el('div', {
          className: 'barn2-wc-product-table-block__options'
        }, [el(ProductTableColumns, {
          columns: attributes.columns,
          saveColumns: function saveColumns(newColumns) {
            //console.log( changed );
            setAttributes({
              columns: newColumns
            });
          ...
          attributes: attributes,
          saveFilters: function saveFilters(newFilters) {
            //console.log( changed );
            setAttributes({
              filters: newFilters
            });
          ...
          try {
            for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
              ...
              if (setting.key === 'additional') {
                attrs = "".concat(setting.value, " ") + attrs;
