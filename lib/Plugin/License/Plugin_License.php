<?php
namespace Barn2\Lib\Plugin\License;

use Barn2\Lib\Registerable,
    Barn2\Lib\Util,
    Barn2\Lib\Plugin\License\License,
    Barn2\Lib\Plugin\License\License_API,
    DateTime;
...
class Plugin_License implements Registerable, License {
    ...
    public function get_status_help_text() {
        ...
        switch ( $this->get_status() ) {
            case 'active':
                $message = __( 'Your license key is active.', 'block-for-woo-product-table' );
                break;
            case 'inactive':
                if ( ! $message ) {
            ...
            default:
                if ( ! $this->get_license_key() ) {
                    $message = __( 'Please enter your license key.', 'block-for-woo-product-table' );
                } elseif ( ! $message ) {
                    $message = __( 'Your license key is invalid.', 'block-for-woo-product-table' );
                }
                break;
        }
    ...
    public function get_error_message() {
        ...
        switch ( $this->get_error_code() ) {
            case 'missing' :
                ...
                $message = sprintf(
                    __( 'Invalid license key - please check your order confirmation email or %1$sAccount%2$s.', 'block-for-woo-product-table' ),
                    Util::format_store_link_open( 'account' ),
                    '</a>'
                );
                break;
            case 'missing_url' :
                $message = __( 'No URL was supplied for activation, please contact support.', 'block-for-woo-product-table' );
                break;
            case 'key_mismatch':
                $message = __( 'License key mismatch, please contact support.', 'block-for-woo-product-table' );
                break;
            case 'license_not_activable' :
                $message = __( 'This license is for a bundled product and cannot be activated.', 'block-for-woo-product-table' );
                break;
            case 'item_name_mismatch' :
            case 'invalid_item_id':
                $message = __( 'Your license key is not valid for this plugin.', 'block-for-woo-product-table' );
                break;
            case 'no_activations_left':
                $limit   = '';

                if ( isset( $license_info['max_sites'] ) ) {
                    $limit = sprintf( _n( '%s site active', '%s sites active', absint( $license_info['max_sites'] ), 'block-for-woo-product-table' ), $license_info['max_sites'] );
                }

                $message        = sprintf( __( 'Your license key has reached its activation limit (%s).', 'block-for-woo-product-table' ), $limit );
                $read_more_link = Util::format_store_link( 'kb/license-key-problems', __( 'Read more', 'block-for-woo-product-table' ) );

                /* translators: support for RTL. 1: license limit error message, 2: a read more link */
                $message = sprintf( __( '%1$s %2$s', 'block-for-woo-product-table' ), $message, $read_more_link );
                break;
            case 'inactive':
            case 'site_inactive':
                $message = $this->get_license_inactive_message();
            ...
            case 'expired' :
                ...
                if ( ! empty( $license_info['expires'] ) && is_numeric( substr( $license_info['expires'], 0, 4 ) ) ) {
                    if ( $expiry_datetime = DateTime::createFromFormat( 'Y-m-d H:i:s', $license_info['expires'] ) ) {
                        ...
                        $message = sprintf( __( 'Your license key expired on %s.', 'block-for-woo-product-table' ), $expiry_datetime->format( get_option( 'date_format' ) ) );
                    }
                }

                $renewal_link = Util::format_link( $this->get_renewal_url(), __( 'Renew now for 20% discount.', 'block-for-woo-product-table' ), true );

                /* translators: support for RTL. 1: expired license error message, 2: a renewal link */
                $message      = sprintf( __( '%1$s %2$s', 'block-for-woo-product-table' ), $message, $renewal_link );
                break;
            case 'disabled':
                $message      = $this->get_license_disabled_message();
                break;
    ...
    private function get_license_inactive_message() {
        return __( 'Your license key is not active. Please reactivate or save the settings.', 'block-for-woo-product-table' );
    }

    private function get_license_expired_message() {
        return __( 'Your license key expired has expired.', 'block-for-woo-product-table' );
    }

    private function get_license_disabled_message() {
        return sprintf(
            __( 'Your license key has been disabled. Please %1$spurchase a new license key%2$s to continue using the plugin.', 'block-for-woo-product-table' ),
