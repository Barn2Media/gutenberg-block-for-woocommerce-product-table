...
class License_Key_Setting implements Registerable, License_Setting {
        ...
        if ( $this->is_license_action( self::ACTIVATE_KEY ) ) {
            ...
            if ( isset( $license_setting['license'] ) ) {
                ...
                $this->add_settings_message(
                    __( 'License key successfully activated.', 'block-for-woo-product-table' ),
                    __( 'There was an error activating your license key.', 'block-for-woo-product-table' ),
                    $activated
                );
            }
            ...
            $this->add_settings_message(
                __( 'License key successfully deactivated.', 'block-for-woo-product-table' ),
                __( 'There was an error deactivating your license key, please try again.', 'block-for-woo-product-table' ),
                $deactivated
            );
            ...
            $this->add_settings_message(
                __( 'The license key looks good!', 'block-for-woo-product-table' ),
                __( 'There\'s a problem with your license key.', 'block-for-woo-product-table' ),
    ...
    public function get_license_key_setting() {
        $setting = array(
            'title' => __( 'License key', 'block-for-woo-product-table' ),
            'type'  => 'text',
        ...
        if ( $this->is_woocommerce ) {
            $setting['desc_tip'] = __( 'The licence key is contained in your order confirmation email.', 'block-for-woo-product-table' );
        } elseif ( $this->is_edd ) {
            // EDD uses title case for setting names, so let's keep things consistent.
            $setting['title'] = __( 'License Key', 'block-for-woo-product-table' );
    ...
    private function get_license_description() {
        $buttons = array(
            ...
            'activate'   => $this->license_action_button( self::ACTIVATE_KEY, __( 'Activate', 'block-for-woo-product-table' ) ),
            'deactivate' => $this->license_action_button( self::DEACTIVATE_KEY, __( 'Deactivate', 'block-for-woo-product-table' ) )
        );
        ...
        if ( $this->license->is_active() ) {
            ...
            if ( $this->license->is_inactive() && $this->is_license_action( 'deactivate_key' ) ) {
                ...
                $message = __( 'License key deactivated.', 'block-for-woo-product-table' );
            } else {
