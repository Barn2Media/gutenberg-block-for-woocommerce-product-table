...
namespace Barn2\Lib\Plugin\Admin;
...
class Admin_Links implements Registerable, Service {
    ...
    public function add_settings_link( $links ) {
        ...
        array_unshift( $links, sprintf( '<a href="%1$s">%2$s</a>', esc_url( $settings_url ), __( 'Settings', 'block-for-woo-product-table' ) ) );
        return $links;
    }
    ...
    public function add_documentation_link( $links, $file ) {
        ...
        $row_meta = array(
            'docs' => sprintf(
                ...
                esc_attr( sprintf( __( 'View %s documentation', 'block-for-woo-product-table' ), $this->plugin->get_name() ) ),
                esc_html__( 'Docs', 'block-for-woo-product-table' )
            )
        );
